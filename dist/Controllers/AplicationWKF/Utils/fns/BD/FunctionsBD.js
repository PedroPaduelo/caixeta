...
exports.List_Full_raw = exports.List_Full_By_User_Creating = exports.List_Full = exports.Get_By_Id = exports.List_Full_By_Col = exports.List = exports.Delete = exports.UpData = exports.Insert = void 0;
const connectionDBA_1 = __importDefault(require("../../../../../Database/connectionDBA"));
async function Insert(table, dados) {
    try {
        const result = await (0, connectionDBA_1.default)(table).insert(dados, ['id']);
...
async function UpData(table, id, dados) {
    try {
        const result = await (0, connectionDBA_1.default)(table).where('id', id).update(dados);
        if (result) {
...
async function Delete(table, id) {
    try {
        const result = await (0, connectionDBA_1.default)(table).where('id', id).del();
        return ({
            status: "success",
            result: result,
...
async function List(table) {
    try {
        const result = await connectionDBA_1.default.select("*").from(table);
        return ({
            status: "success",
            result: result,
...
async function List_Full_By_Col(table, col, col_value) {
    try {
        const result = await connectionDBA_1.default.select("*")
            .from(table)
            .where(col, col_value)
            .orderBy(col);
        return ({
...
async function Get_By_Id(table, id) {
    try {
        const result = await (0, connectionDBA_1.default)(table).where('id', id).first();
        if (result) {
            return ({
                status: "success",
                result: result,
...
async function List_Full_raw(table) {
    try {
        const result = await connectionDBA_1.default.raw(`select * from ${table}`);
        return ({
            status: "success",
            result: result,
            message: "Sucesso ao listar!!!"
